...
def driver():
    ...
    if args.authentication_model == "both":
        ...
        while True:
            while not bothAuthenticated:
                ...
                recognizedFaces = fr.run(data, vs, fps, debug=True)
                ...
                userSet = set.union(set.union(recognizedFaces,recognizedVoice), validUsers)
                if len(userSet) == 1:
    ...
    elif args.authetication_model == "voice":
        ...
        while True:
            while not voiceAuthenticated:
                ...
                #else: continue
                if args.unlock_phrase != "" and not vr.recognizeWord(): continue
                userSet = set.union(recognizedVoice, validUsers)
                if len(userSet) == 1:
            ...
            while state == "unlocked":
                if lock button pressed:
                    lock()
                    voiceAuthenticated = False
    elif args.authetication_model == "face":
        ...
        while True:
            while not faceAuthenticated:
                ...
                recognizedFaces = fr.run(data, vs, fps, debug=True)
                # Turn on green led for video
                userSet = set.union(recognizedFaces, validUsers)
                if len(userSet) == 1:
